...
如果你不想一上来就设计一整套规则体系，可以先按这个“最小可用”落地：

1. 在项目根目录创建 `.cursor/rules/`。
2. 新建 `index.mdc`（小文件，`alwaysApply: true`），只写“规则地图”（每条规则的用途、触发方式、关键词）。
3. 把你现有的“大规则”拆成 2～6 个中等规则文件（每个聚焦一个主题），全部 **不设** `alwaysApply: true`。
4. 对“能用文件路径判断”的规则加 `globs`（例如 Vue、测试、API 目录等）；对“场景化/工具类”的规则不写 globs，只在索引里标关键词。
5. 开始使用：当你发现 AI 被无关规则干扰或上下文变慢，就把那条规则从 always 移到按需，并在索引里补关键词。

这套做法的核心是：**索引永远小、规则永远可拆、加载永远按需**。
...
## 四、Cursor 规则 front matter：和动态加载相关的字段
...
索引不需要长篇大论，重点是让“人和 AI 都能快速决策要加载哪条”。一个实用模板如下（示意）：

```md
## 规则索引（index.mdc）

### Always（始终加载）
- global-development-standards.mdc：命名/目录/提交规范（小）

### Auto Attached（按文件路径）
- vue-development-patterns.mdc（globs: src/**/*.vue）：Composition API、组件约定（中）
- api-integration-workflow.mdc（globs: src/api/**）：接口封装、错误处理、字段对齐（中）

### Manual / Keyword（按需）
- TableCommon-项目规则.mdc：表格/列表配置化（大；关键词：TableCommon / listData / 列表页）
- api-fox-patterns.mdc：Apifox 查询与联调（小；关键词：Apifox / mock / 字段对接）
```

建议你在索引里固定三类信息：

- **触发方式**：always / globs / keyword
- **适用场景**：一句话即可
- **关键词**：3～5 个，尽量贴近你对话里会说的词
...
- **规则没自动挂载**：优先检查 `globs` 是否过宽/过窄、路径分隔符是否正确、文件是否在匹配目录下；其次检查 `.mdc` 顶部 YAML 是否格式正确（缩进/引号）。
- **对话变慢或上下文溢出**：确认是否把大文件设成了 `alwaysApply: true`；把“大规则”拆分，并把常用小规则留在 L2（按 globs），大规则放 L3（按关键词）。
- **规则互相打架**：把“必须遵守的全局底线”放在 L1；其余规则避免写成“覆盖式”，尽量写成“优先级与取舍”（例如：性能优先 vs 可读性优先的边界）。
